...
class MySeat(APIView):
    def get(self, request, format=None):
        ...
        try:
            using_seat = models.Seat.objects.get(
                now_user=user, end_datetime__gte=now, usable=True)
        except models.Seat.DoesNotExist:
            return Response(status=status.HTTP_204_NO_CONTENT)

        serializer = serializers.SeatSerializer(using_seat)
...
class Allocation(APIView):
    def put(self, request, seat_id, user_id, format=None):

        # one_day_to_seconds = 86400
        # membership_end_date = datetime.strptime(request.data['end_date'],
        #                                         '%Y-%m-%d %H:%M:%S')
        # one_day_after = datetime.now() + timedelta(hours=24)
        # delta = membership_end_date - one_day_after
        # if delta.seconds > one_day_to_seconds:  # 하루 좌석 잡을 경우의 종료시각보다 멤버십이 더 길게 있을 경우
        #     target_end_datetime = one_day_after
        # else:
        #     target_end_datetime = membership_end_date
        # print(delta.seconds)

        # 만료시각이 하루 이하이면 만료시각으로 자리 잡도록 함

        end_datetime = datetime.strptime(request.data['end_datetime'],
                                         '%Y-%m-%d %H:%M:%S')
        ...
        # 성별을 통한 배정 이미지 찾기
        if target_user.gender is None:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        else:
            seat_images = models.SeatImage.objects.filter(
                gender=target_user.gender)

        count_images = len(seat_images)
        rand_limit = count_images - 1
        random_image_number = randint(0, rand_limit)
        target_seat_img = seat_images[random_image_number]  #선택된 이미지
        ...
        try:
            target_action = models.Action.objects.get(
                en_substance='allocation')
        except models.Action.DoesNotExist:
            return Response(status=status.HTTP_400_BAD_REQUEST)

        target_seat.seat_image = target_seat_img
        target_seat.now_user = target_user
        target_seat.end_datetime = end_datetime
...
class ReturnSeat(APIView):
    ...
    def put(self, request, seat_id, format=None):
        ...
        try:
            target_seat_img = models.SeatImage.objects.get(
                action=target_action)
        except models.Action.DoesNotExist:
            return Response(status=status.HTTP_400_BAD_REQUEST)

        target_seat.end_datetime = datetime.now()
        target_seat.seat_image = target_seat_img
        target_seat.now_user = None
